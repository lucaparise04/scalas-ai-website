      ...
      <Container>
        ...
        >
          ...
          {columns.map((column, colIdx) => (
            <div key={colIdx} className="hidden lg:flex flex-col gap-6">
              {column.map((item, i) => (
                <motion.div key={`${colIdx}-${i}`} variants={staggerItem}>
          ...
          {TESTIMONIALS.map((item, i) => (
            <motion.div key={i} variants={staggerItem} className="lg:hidden">
              <Card className="flex flex-col">
                <svg
                  className="w-8 h-8 text-purple/40 mb-4 flex-shrink-0"
                  fill="currentColor"
